...
def create_train_model(
    ...
    with graph.as_default():
        ...
        iterator = iterator_utils.get_iterator(
            ...
            sent_feature_dataset=sent_feat_dataset,
...
def create_emb_for_encoder_and_decoder_and_sent_feat(share_vocab,
                                                     src_vocab_size,
                                                     tgt_vocab_size,
                                                     src_embed_size,
                                                     tgt_embed_size,
    ...
    """Create embedding matrix for encoder and decoder.
    ...
    with tf.variable_scope(
        ...
        if share_vocab:
            ...
            if sent_feat_vocab_size:
                sent_feat_embedding_encoder = tf.get_variable(
                    "sent_feat_embedding_encoder", [sent_feat_vocab_size, sent_feat_embed_size])
        else:
...
def compute_perplexity(model, sess, name):
    ...
    try:
        ...
        perplexity = utils.safe_exp(total_loss / 0.00001)
        print("ATTENTION: total_predict_count is zero")
    utils.print_time("  eval %s: perplexity %.2f" % (name, perplexity),
                     start_time)
    return perplexity, total_loss / total_predict_count
